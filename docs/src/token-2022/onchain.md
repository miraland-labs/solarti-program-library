...
This guide requires the Miraland CLI tool suite, minimum version 1.10.33 in order
...
```console
$ miraland program dump Token8N5ecJeFxL83iFa2h7AgJ8AtufM7bbg63LrW89 spl_token_2022.so
```
...
use miraland_sdk::{system_instruction, transaction::Transaction};
...
incur 1 byte of overhead because of the deduplication of accounts in the Miraland
transaction format.
...
next stage of DeFi on Miraland.
...
```rust
use miraland_program::{clock::Clock, sysvar::Sysvar};
