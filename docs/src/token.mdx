...
The `solarti-token` configuration is shared with the `solana` command-line tool.
...
```
### Keypair
You can either get your keypair using [`Keypair`](https://solana-labs.github.io/solana-web3.js/classes/Keypair.html) from `@solarti/web3.js`, or let the user's wallet handle the keypair and use `sendTransaction` from [`wallet-adapter`](https://github.com/solana-labs/wallet-adapter)
...
Account, all remaining MLN will be transferred to another Solana account
