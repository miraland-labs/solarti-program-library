...
A Token program on the Miraland blockchain.
...
The Token Program is written in Rust and available on [crates.io](https://crates.io/crates/solarti-token) and [docs.rs](https://docs.rs/solarti-token).

Auto-generated C bindings are also available
[here](https://github.com/miraland-labs/miraland-program-library/blob/master/token/program/inc/token.h)

[JavaScript
bindings](https://github.com/miraland-labs/miraland-program-library/tree/master/token/js)
...
The `solarti-token` command-line utility can be used to experiment with SPL
tokens.  Once you have [Rust installed](https://rustup.rs/), run:
```console
$ cargo install solarti-token-cli
```

Run `solarti-token --help` for a full description of available commands.

### Configuration

The `solarti-token` configuration is shared with the `miraland` command-line tool.
...
#### Yarn
```console
yarn add @solarti/solarti-token
```
#### npm
```console
npm install @solarti/solarti-token
```
### Configuration
You can connect to different clusters using `Connection` in `@solarti/web3.js`
```typescript
const web3 = require('@solarti/web3.js');
const connection = new web3.Connection(web3.clusterApiUrl('devnet'), 'confirmed');
```
### Keypair
You can either get your keypair using [`Keypair`](https://miraland-labs.github.io/miraland-web3.js/classes/Keypair.html) from `@solarti/web3.js`, or let the user's wallet handle the keypair and use `sendTransaction` from [`wallet-adapter`](https://github.com/miraland-labs/wallet-adapter)
...
Creating tokens and accounts requires MLN for account rent deposits and
transaction fees. If the cluster you are targeting offers a faucet, you can get
a little MLN for testing:
...
```jsx
import { createMint } from '@solarti/solarti-token';
import { clusterApiUrl, Connection, Keypair, LAMPORTS_PER_MLN } from '@solarti/web3.js';
...
Tokens when initially created by `solarti-token` have no supply:

<Tabs className="unique-tabs" groupId="language-selection">
  <TabItem value="cli" label="CLI" default>

```console
$ solarti-token supply AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM
0
```
...
```console
$ solarti-token create-account AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM
...
```console
$ solarti-token balance AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM
0
```
...
```console
$ solarti-token mint AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM 100
Minting 100 tokens
...
```jsx
import {AccountLayout, TOKEN_PROGRAM_ID} from "@solarti/solarti-token";
import {clusterApiUrl, Connection, PublicKey} from "@solarti/web3.js";
...
### Example: Wrapping MLN in a Token

When you want to wrap MLN, you can send MLN to an associated token account
on the native mint and call `syncNative`. `syncNative` updates the `amount`
field on the token account to match the amount of wrapped MLN available.
That MLN is only retrievable by closing the token account and choosing
...
```console
$ solarti-token wrap 1
Wrapping 1 MLN into GJTxcnA5Sydy8YRhqvHxbQ5QNsPyRKvzguodQEaShJje
...
```jsx
import {NATIVE_MINT, createAssociatedTokenAccountInstruction, getAssociatedTokenAddress, createSyncNativeInstruction, getAccount} from "@solarti/solarti-token";
import {clusterApiUrl, Connection, Keypair, LAMPORTS_PER_MLN, SystemProgram, Transaction, sendAndConfirmTransaction} from "@solarti/web3.js";
...
// Create token account to hold your wrapped MLN
...
// Transfer MLN to associated token account and use SyncNative to update wrapped MLN balance
...
To unwrap the Token back to MLN:

<Tabs className="unique-tabs" groupId="language-selection">
  <TabItem value="cli" label="CLI" default>

```console
$ solarti-token unwrap GJTxcnA5Sydy8YRhqvHxbQ5QNsPyRKvzguodQEaShJje
...
First the receiver uses `solarti-token create-account` to create their associated
token account for the Token type.  Then the receiver obtains their wallet
address by running `miraland address` and provides it to the sender.
...
```console
$ solarti-token transfer AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM 50 vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg
Transfer 50 tokens
...
```jsx
import { clusterApiUrl, Connection, Keypair, LAMPORTS_PER_MLN } from '@solarti/web3.js';
import { createMint, getOrCreateAssociatedTokenAccount, mintTo, transfer } from '@solarti/solarti-token';
...
The receiver obtains their wallet address by running `miraland address` and provides it to the sender.
...
```console
$ solarti-token transfer --fund-recipient AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM 50 vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg
Transfer 50 tokens
  Sender: 7UX2i7SucgLMQcfZ75s3VXmZZY4YRUyJN9X1RgfMoDUi
  Recipient: vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg
  Recipient associated token account: F59618aQB8r6asXeMcB9jWuY6NEx1VduT9yFo1GTi1ks
  Funding recipient: F59618aQB8r6asXeMcB9jWuY6NEx1VduT9yFo1GTi1ks (0.00203928 MLN)
...
```console
$ solarti-token create-account AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM /path/to/auxiliary_keypair.json
...
```console
$ solarti-token accounts AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM -v
...
```console
$ solarti-token transfer AQoKYV7tYpTrFZN6P5oUufbQKAUr9mNYGe1TTJC9wajM 50 CqAxDdBRnawzx9q4PYM3wrybLHBhDZ4P6BTV13WsRJYJ
Transfer 50 tokens
  Sender: 7UX2i7SucgLMQcfZ75s3VXmZZY4YRUyJN9X1RgfMoDUi
...
```jsx
import {getAccount, createMint, createAccount, mintTo, getOrCreateAssociatedTokenAccount, transfer} from "@solarti/solarti-token";
import {clusterApiUrl, Connection, Keypair, LAMPORTS_PER_MLN} from "@solarti/web3.js";
...
```console
$ solarti-token create-token --decimals 0
...
```console
$ solarti-token create-account 559u4Tdr9umKwft3yHMsnAxohhzkFnUBPAFtibwuZD9z
...
```console
$ solarti-token mint 559u4Tdr9umKwft3yHMsnAxohhzkFnUBPAFtibwuZD9z 1 7KqpRwzkkeweW5jQoETyLzhvs9rcCj9dVQ1MnzudirsM
Minting 1 tokens
...
```console
$ solarti-token authorize 559u4Tdr9umKwft3yHMsnAxohhzkFnUBPAFtibwuZD9z mint --disable
...
```console
$ solarti-token account-info 559u4Tdr9umKwft3yHMsnAxohhzkFnUBPAFtibwuZD9z
...
```console
$ solarti-token supply 559u4Tdr9umKwft3yHMsnAxohhzkFnUBPAFtibwuZD9z
1
```
...
The main difference in `solarti-token` command line usage when referencing multisig
...
Multisig accounts can be used for any authority on an Solarti Token mint or token
account.
<ul>
  <li>Mint account mint authority:
    <code>solarti-token mint ...</code>,
    <code>solarti-token authorize ... mint ...</code>
  </li>
  <li>Mint account freeze authority:
    <code>solarti-token freeze ...</code>,
    <code>solarti-token thaw ...</code>,
    <code>solarti-token authorize ... freeze ...</code>
  </li>
  <li>Token account owner authority:
    <code>solarti-token transfer ...</code>,
    <code>solarti-token approve ...</code>,
    <code>solarti-token revoke ...</code>,
    <code>solarti-token burn ...</code>,
    <code>solarti-token wrap ...</code>,
    <code>solarti-token unwrap ...</code>,
    <code>solarti-token authorize ... owner ...</code>
  </li>
  <li>Token account close authority:
    <code>solarti-token close ...</code>,
    <code>solarti-token authorize ... close ...</code>
  </li>
</ul>
...
Multisig accounts can be used for any authority on an Solarti Token mint or token account.
...
```console
$ for i in $(seq 3); do miraland-keygen new --no-passphrase -so "signer-${i}.json"; done
...
```console
$ for i in $(seq 3); do SIGNER="signer-${i}.json"; echo "$SIGNER: $(miraland-keygen pubkey "$SIGNER")"; done
...
Now the multisig account can be created with the `solarti-token create-multisig`
...
NOTE: Solarti Token Multisig accounts are limited to a signer-set of eleven signers
...
```
$ solarti-token create-multisig 2 BzWpkuRrwXHq4SSSFHa8FJf6DRQy4TaeoXnkA89vTgHZ \
...
$ solarti-token create-account 4VNVRJetwapjwYU8jf4qPgaCeD76wyz8DuNj8yMCQ62o
...
$ solarti-token authorize 4VNVRJetwapjwYU8jf4qPgaCeD76wyz8DuNj8yMCQ62o mint 46ed77fd4WTN144q62BwjU2B3ogX3Xmmc8PT5Z3Xc2re
...
```
$ solarti-token mint 4VNVRJetwapjwYU8jf4qPgaCeD76wyz8DuNj8yMCQ62o 1 EX8zyi2ZQUuoYtXd4MKmyHYLTjqFdWeuoTHcsTdJcKHC \
...
previous examples of [multisig](#example-mint-with-multisig-authority) with [offline signing](https://docs.miraland.io/offline-signing)
and a [nonce account](https://docs.miraland.io/offline-signing/durable-nonce).
...
```console
$ miraland nonce-account Fjyud2VXixk2vCs4DkBpfpsq48d81rbEzh6deKt7WvPj
Balance: 0.01 MLN
Minimum Balance Required: 0.00144768 MLN
...
There is a rich set of JSON RPC methods available for use with Solarti Token:
...
See https://docs.miraland.io/apps/jsonrpc-api for more details.

Additionally the versatile `getProgramAccounts` JSON RPC method can be employed in various ways to fetch Solarti Token accounts of interest.
...
```
curl http://api.mainnet-beta.miraland.io -X POST -H "Content-Type: application/json" -d '
  {
    "jsonrpc": "2.0",
    "id": 1,
...
Account](https://github.com/miraland-labs/miraland-program-library/blob/08d9999f997a8bf38719679be9d572f119d0d960/token/program/src/state.rs#L86-L106)s,
and then the `"memcmp": ...` filter selects based on the
[mint](https://github.com/miraland-labs/miraland-program-library/blob/08d9999f997a8bf38719679be9d572f119d0d960/token/program/src/state.rs#L88)
...
Account](https://github.com/miraland-labs/miraland-program-library/blob/08d9999f997a8bf38719679be9d572f119d0d960/token/program/src/state.rs#L86-L106)s,
and then the `"memcmp": ...` filter selects based on the
[owner](https://github.com/miraland-labs/miraland-program-library/blob/08d9999f997a8bf38719679be9d572f119d0d960/token/program/src/state.rs#L90)
address within each token account.
...
the Miraland account being initialized also be a signer. The `InitializeMint`
instruction should be atomically processed with the system instruction that
creates the Miraland account by including both instructions in the same
transaction.
...
the Miraland account being initialized also be a signer. The `InitializeAccount`
instruction should be atomically processed with the system instruction that
creates the Miraland account by including both instructions in the same
transaction.
...
does not necessarily imply that the involved Accounts were valid Solarti Token
...
require the Miraland account being initialized also be a signer. The
`InitializeMultisig` instruction should be atomically processed with the system
instruction that creates the Miraland account by including both instructions in
...
The Token Program can be used to wrap native MLN. Doing so allows native MLN to
...
Accounts containing wrapped MLN are associated with a specific Mint called the
"Native Mint" using the public key
`MLN1111111111111111111111111111111111111111`.

These accounts have a few unique behaviors

- `InitializeAccount` sets the balance of the initialized Account to the MLN
  balance of the Miraland account being initialized, resulting in a token balance
  equal to the MLN balance.
- Transfers to and from not only modify the token balance but also transfer an
  equal amount of MLN from the source account to the destination account.
- Burning is not supported
- When closing an Account the balance may be non-zero.

The Native Mint supply will always report 0, regardless of how much MLN is currently wrapped.

### Rent-exemption

To ensure a reliable calculation of supply, a consistency valid Mint, and
consistently valid Multisig accounts all Miraland accounts holding an Account,
Mint, or Multisig must contain enough MLN to be considered [rent
exempt](https://docs.miraland.io/implemented-proposals/rent)

### Closing accounts

An account may be closed using the `CloseAccount` instruction. When closing an
Account, all remaining MLN will be transferred to another Miraland account
...
This section describes how to integrate Solarti Token support into an existing
wallet supporting native MLN.  It assumes a model whereby the user has a single
system account as their **main wallet address** that they send and receive MLN
from.

Although all Solarti Token accounts do have their own address on-chain, there's no
need to surface these additional addresses to the user.

There are two programs that are used by the wallet:
* Solarti Token program: generic program that is used by all Solarti Tokens
...
The [getTokenAccountsByOwner](https://docs.miraland.io/apps/jsonrpc-api#gettokenaccountsbyowner)
...
on-chain, requiring a small amount of MLN to mark the account as rent-exempt.
...
a given Solarti Token itself before indicating to the user that they are able to
receive that Solarti Tokens type (typically done by showing the user their receiving
address). A wallet that chooses to not perform this step may limit its user's ability
to receive Solarti Tokens from other wallets.

#### Sample "Add Token" workflow
The user should first fund their associated token account when they want to
receive Solarti Tokens of a certain type to:
...
much MLN it will cost to add the token.
...
At any time ownership of an existing Solarti Token account may be assigned to the
user.  One way to accomplish this is with the
`solarti-token authorize <TOKEN_ADDRESS> owner <USER_ADDRESS>` command.  Wallets
...
1. If the recipient's associated token account does not yet exist, the sender
   ...
   creation the transfer will require more MLN than normal.
...
* [solarti-token-registry](https://www.npmjs.com/package/@solarti/solarti-token-registry)
package, maintained at [https://github.com/miraland-labs/token-list](https://github.com/miraland-labs/token-list)
...
* If the user is the close authority for the ancillary account, the wallet can
  reclaim MLN for the user by closing the account.
* If the ancillary account was funded by a 3rd party, once the account is
  emptied that 3rd party may close the account and reclaim the MLN.
...
- Solarti token and amount to be vested
- recipient
