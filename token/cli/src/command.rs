...
async fn command_create_token(
    ...
    if enable_metadata {
        println_display(
            ...
            format!(
                ...
                `solarti-token initialize-metadata {token_pubkey} <YOUR_TOKEN_NAME> <YOUR_TOKEN_SYMBOL> <YOUR_TOKEN_URI>`, \
    ...
    if enable_group {
        println_display(
            config,
            format!(
                "To initialize group configurations inside the mint, please run `solarti-token initialize-group {token_pubkey} <MAX_SIZE>`, and sign with the mint authority.",
            ),
        );
    }
    ...
    if enable_member {
        println_display(
            config,
            format!(
                "To initialize group member configurations inside the mint, please run `solarti-token initialize-member {token_pubkey}`, and sign with the mint authority and the group's update authority.",
            ),
        );
    }
