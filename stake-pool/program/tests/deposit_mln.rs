...
async fn success(token_program_id: Pubkey) {
    ...
    let tokens_issued_user =
        tokens_issued - stake_pool_accounts.calculate_mln_deposit_fee(tokens_issued);
...
#[tokio::test]
async fn success_with_mln_deposit_authority() {
    ...
    let mut transaction = Transaction::new_with_payer(
        &[instruction::set_funding_authority(
            ...
            Some(&mln_deposit_authority.pubkey()),
...
#[tokio::test]
async fn fail_without_mln_deposit_authority_signature() {
    let (mut banks_client, payer, recent_blockhash) = program_test().start().await;
    let mln_deposit_authority = Keypair::new();
    let stake_pool_accounts = StakePoolAccounts::default();
    stake_pool_accounts
    ...
    match error {
        TransactionError::InstructionError(_, InstructionError::Custom(error_index)) => {
            assert_eq!(
                error_index,
                error::StakePoolError::InvalidMlnDepositAuthority as u32
            );
        }
...
async fn success_with_referral_fee() {
    ...
    let referrer_balance_post =
        get_token_balance(&mut context.banks_client, &referrer_token_account.pubkey()).await;
    let referral_fee = stake_pool_accounts.calculate_mln_referral_fee(
        stake_pool_accounts.calculate_mln_deposit_fee(TEST_STAKE_AMOUNT),
    );
...
async fn success_with_slippage(token_program_id: Pubkey) {
    ...
    let new_pool_tokens = pre_stake_pool
        ...
        .unwrap();
    let pool_tokens_mln_deposit_fee = pre_stake_pool
        .calc_pool_tokens_mln_deposit_fee(new_pool_tokens)
        .unwrap();
    let tokens_issued = new_pool_tokens - pool_tokens_mln_deposit_fee;

    // Fail with 1 more token in slippage
    let error = stake_pool_accounts
        .deposit_mln_with_slippage(
            &mut context.banks_client,
            &context.payer,
            &context.last_blockhash,
            &pool_token_account,
