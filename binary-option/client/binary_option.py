...
ASSOCIATED_TOKEN_ACCOUNT_PROGRAM_ID = 'ATAccPjxdgWfJKKN4PmfJ55FbEDEwD8zJUwVjuL9MuHy'
TOKEN_PROGRAM_ID = 'Token4Q2B47VCdUy8u3rSTMMk2bGA1k7eN8qfKSzdiM'
BINARY_OPTION_PROGRAM_ID = 'bopttqXMCA1EctcDSXLzjbP672NLu1667tENLbD9tLL'
        ...
        try:
            ...
            try:
                if amount is None:
                    ...
                    lamports = int(amount)
                msg += f" | Fetched lamports: {lamports * 1e-9} MLN"
            except Exception as e:
            ...
            try:
                ...
                return json.dumps(
                    {
                        'status': HTTPStatus.OK,
                        'msg': f"Successfully sent {lamports * 1e-9} MLN to {to}",
                        'tx': response.get('result') if skip_confirmation else response['result']['transaction']['signatures'],
                    }
                )
            except Exception as e:
